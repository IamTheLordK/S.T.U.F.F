...
Input.InputBegan:Connect(function(Key)
 ...
 elseif Key.KeyCode == Enum.KeyCode.R and Input:IsKeyDown(Enum.KeyCode.LeftControl) then
...
while game:GetService("RunService").RenderStepped:Wait() do
 ...
 for _, P in ipairs(Players:GetPlayers()) do
  ...
  if TeamCheck and P.Team == Players.LocalPlayer.Team then continue end
  local D = P:DistanceFromCharacter(Char.Head.Position)
  if D <= 0 or D > MaxD or D >= MinD - 5 then continue end
